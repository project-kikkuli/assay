...
| Does test order change how fast an agent sees the first real failure? | [Failure ordering](experiments/failure-ordering/): 20 real regression/fix pairs mined from pytest and click's own history; a real drop-in pytest plugin combining recent-failure, proximity-to-diff, and duration beats default collection order in 19/20 cases, median 69 vs. 1,936 tests run before the real failure. |
| Can a real mechanism find the risky test first, not just measure that one would? | [pytest-order-plugin](experiments/pytest-order-plugin/): a standalone, drop-in pytest plugin (`--assay-order`, `--assay-fail-fast`) reading pytest's own lastfailed cache, a real `git diff`, and its own persisted duration history; 5 real subprocess integration tests pass, and it wins 19/20 on the mined sample above. |
| How much of a failing CI log does an agent need to read to localize it? | [Failure context budget](experiments/failure-context-budget/): across 112 real failures from the same 20-case sample, reading a real log from the bottom instead of the top cuts the median lines needed to identify the failing test 346x; a real tail-first/junit-only extractor tool finds all 112. |
...
| How many real CI rounds does a PR actually take to go green, and why is a round red? | [CI convergence](experiments/ci-convergence/): 100 real merged PRs across 5 busy public repos, 78.4% green on the first round; of real red-round causes, lint/format and type-check together are 56.5%, test failures 39.1%. |
...
| Does real parallel batching actually cut real merge-queue wall time? | [Real batch queue](experiments/real-batch-queue/): a real multi-worker scheduler over real `git worktree` checkouts, with a real injected code mutation and real recursive bisection. Plain parallel dispatch (no batching) is the fastest real wall-clock option; real batching uses fewer real jobs and less real CPU time but real makespan is *worse*, from real worker load imbalance a single-lane simulator can't produce. |
| Does a static selector miss real regressions? | [Task selector CLI](experiments/task-selector-cli/): a runnable selector validated against `failure-ordering`'s 4 real regressions. The naive `test_*.py` manifest misses 3/4 (real test files under `testing/python/` don't follow that convention); reading the repo's own real `python_files` config fixes all 3, for a validated 0% miss rate. |
