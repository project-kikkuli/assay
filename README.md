...
| What does a merge queue buy, and does selection narrowness change it? | [Merge-queue safety](experiments/merge-queue-safety/): 0/90 real pytest merges showed a PR head that collected cleanly while the real merged tree did not; a seeded disjoint-change conflict confirms change-scoped selection misses exactly this case while a merge-queue check and concurrency-aware selection both catch it. |
