...
| Does test order change time to the first real failure, at scale? | [Failure ordering](experiments/failure-ordering/results_sample_pytest.json): 20 real regression/fix pairs mined from pytest + click's own history ([pytest](experiments/failure-ordering/results_sample_pytest.json), [click](experiments/failure-ordering/results_sample_click.json)); pooled median tests-run-before-stop is **1,936** for default order, **77** for proximity alone, **69** for [a real drop-in plugin](experiments/pytest-order-plugin/results_replay_pytest.json) combining recent-failure + proximity + duration, winning **19/20** cases (only real-plugin replay shown; isolated single-factor medians in the README). | Proximity-to-the-diff is the strongest single lever on this sample; a real, general-purpose plugin combining all three factors is a ~28x median reduction over default and is not sample-specific code — it is a standalone pytest plugin any repo can install. |
| How much of a failing log does localizing the failure actually need, at scale? | [Failure context budget](experiments/failure-context-budget/results_sample_pytest.json): across 112 real failures from the same 20-case sample ([pytest](experiments/failure-context-budget/results_sample_pytest.json), [click](experiments/failure-context-budget/results_sample_click.json)), finding which test failed needs a median of **2,076** lines read from the top vs. **6** from the bottom — a 346x reduction. A real tail-first + junit-failures-only extractor tool (`localize.py`) finds all 112 real failures (recall 1.0). | Almost the entire log is noise if read top-down, across two unrelated real codebases, not just one bug. A small, real, general-purpose tool — not a per-project script — recovers the signal a naive whole-log or whole-JUnit-XML dump buries. |
...
| Does real parallel batching cut real merge-queue wall time? | [Real batch queue](experiments/real-batch-queue/results.json): a real multi-worker scheduler over real `git worktree` checkouts, with a real injected mutation (`hmac.compare_digest` negated) and real recursive `pytest` bisection, 24 real `itsdangerous` commits, 2 really broken, 4 real workers. Plain parallel dispatch (no batching): **8.55s** makespan, **3.29s** mean convergence. Real batching (size 6): fewer real jobs (16 vs 24) and less real CPU (17.98s vs 21.18s) but a **worse** real makespan (**10.36s**) — held across 3 real runs. Serial: **18.85s** makespan. | A real scheduler surfaced a real load-imbalance effect (stragglers on batches containing the mutation) that a single-lane simulator has no mechanism to produce: fine-grained parallel dispatch self-balances across workers; coarse batches don't once one fails. |
| Does a static explicit-input selector miss real regressions? | [Task selector CLI](experiments/task-selector-cli/results.json): a runnable CLI, validated against the same 4 real pytest regressions `failure-ordering` established. `diff-affected-fraction`'s naive `test_*.py` manifest misses **3/4** — the real regression test files live under `testing/python/`, which pytest's own real `pyproject.toml` declares as tests (`python_files = [..., "testing/python/*.py"]`) but the naive glob never reads. Reading that real config fixes all 3: **0/4 misses**, mean **60.7%** fewer real tests selected than the real 4,543-test full suite (real exec times 0.80s-90.92s depending on diff breadth). | A validated selector beats an asserted one: the naive heuristic looked reasonable and silently missed most of a small real regression population until checked against real failures. |
