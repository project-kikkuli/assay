...
from run import HERE, REVISION, Lab
...
def main():
    ...
    report = {"revision": REVISION, "scope": "Post-challenge requirements; these are no longer blind faults",
              "records": [], "matrix": [], "oracle_sha256": hashlib.sha256((HERE / "lifecycle_tests.py").read_bytes()).hexdigest()}
    ...
    report["expected_outcomes_observed"] = report["oracle_unchanged"] and len(report["matrix"]) == 2 and all(row["outcome"] == "caught" for row in report["matrix"])
